...
mutable struct Simulation
   ...
   function Simulation(m::jlModel, d::jlData, window::GLFW.Window)
      new(0.0, 0.0, false, false, false, GLFW.MOUSE_BUTTON_1, 0.0,
          ...
          Ref(mjvFigure()),
          m, d, window,
          GLFW.GetVideoMode(GLFW.GetPrimaryMonitor())
         )
   end
end
...
const keycmds = Dict{GLFW.Key, Function}(GLFW.KEY_F1=>(s)->begin  # help
                                         ...
                                         GLFW.KEY_V=>(s)->begin
                                            if s.record == nothing
                                               #println("Saving binary data to video.bin")
                                               #s.record = open("/tmp/video.bin","w")
...
# show sensor figure
function sensorshow(s::Simulation, rect::mjrRect)
...
function start(mm::jlModel, dd::jlData, width=1200, height=900) # TODO named args for callbacks
   ...
   mjv_makeScene(C_NULL, s.scn, maxgeom)
   mjv_defaultPerturb(s.pert)
   mjv_defaultCamera(s.cam)
   mjv_defaultOption(s.vopt)
   mjr_defaultContext(s.con)
   mjr_makeContext(s.m.m, s.con, Int(fontscale)) # model specific setup
